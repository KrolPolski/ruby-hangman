...
class Board
  def initialize(word)
    @word = word.downcase
    @game_over = false
    puts "\nYou can save by pressing CTRL-S at any time. Load with CTRL-L"
    @failures = 0
    @failure_log = []
    @guess_log = []
  ...
  def draw_board
    ...
    puts "Wrong answer collection: #{@failure_log} Save: CTRL-S Load: CTRL-L"
    p @guess.join
  end
